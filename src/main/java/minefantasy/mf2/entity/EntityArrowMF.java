...
public class EntityArrowMF extends EntityArrow implements IProjectile, IDamageType, IArrowRetrieve
{
	...
	public EntityArrowMF(World world, EntityLivingBase shooter, EntityLivingBase target, float accuracy, float power)
	{
		...
		if (d3 >= 1.0E-7D)
		{
			...
			this.setThrowableHeading(d0, d1 + (double) f4, d2, accuracy, power);
		}
	}
	...
	public void setThrowableHeading(double x, double y, double z, float angle, float power)
	{
		...
		x += this.rand.nextGaussian() * (double) (this.rand.nextBoolean() ? -1 : 1)
				* 0.007499999832361937D * (double) power;
	...
	public void onUpdate()
	{
		...
		} else
		{
			...
			this.motionY -= (double) (f1*getGravityModifier());
			this.setPosition(this.posX, this.posY, this.posZ);
			this.func_145775_I();
		}
	}
